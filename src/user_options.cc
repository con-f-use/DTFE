                              ...
                              po::positional_options_description &p)
{
    ...
    mainOptions.add_options()
            ...
            ("input,i", po::value< std::vector<int> >()->multitoken(), "Give the type of the input file, and if present the type of data to read and the particles species to read it from. Type:\n"
                "  101 = gadget binary snapshot (single- or multi-) file\n"
                "  105 = gadget HDF5 file\n"
                "  111 - reads number, box dimensions, particles positions and masses (weights) from a text file.\n"
                "  112 - as above but weights are assumed same for all particles\n"
                "  121 - reads everything from a binary file. Modify this function in file 'src/io/binary_io.cc'\n"
                "  131 - read everything from your custom file. Modify this function in file 'src/io/my_io.cc'\n"
                "See documenation for more options. If present, a 2nd argument gives the data to be read from file:\n"
                "  1 = positions\n"
                "  2 = masses (weights)\n"
                "  4 = velocities\n"
                "  8 = internal energy U\n"
                " 16 = extra block (or if U present but skpped also U)\n"
                " 32 = extra block if U is present but skipped by user\n"
                "So to read positions, masses and velocities insert 1+2+4=7. However, in the original Gadget snapshot format blocks could be missing, so beyond velocities there is no way of automatically telling which block is which. Internal energy is a special block as its value is multiplied by the mass. If you don't read internal energy but a later block the later block will correspond to 16 if no internal energy block is present in the snapshot. If it is, it will correspond to 32 since internal energy will be read as 16 \"unknown gas block\". An optional 3rd argument to the \"--input\"-option gives the particle species to be read from file:\n"
                "  1 = 1st species (gas)\n"
                "  2 = 2nd species (dark matter)\n"
                "  4 = 3rd species (disk stars)\n"
                "  8 = 4th species (bulge stars)\n"
                "  16 = 5th species (newly formed stars)\n"
                "  32 = 6th species (boundary particles)\n"
                "So to read the data of species 2,3 and 4 insert 2+4+8=14. Combined \"--input 101 7 14\" would read positions, velocity and masses for dark matter, disk and new stars from a gadget snapshot.")
    ...
    averagingOptions.add_options()
            ...
            ("density0", po::value<Real>(&averageDensity), "supply a value to be used to scale the density. If none is supplied, the average density will be used for this task. The supplied value must be in input units of mass per square Mpc (since Mpc is the DTFE internal unit of length)")
...
void User_options::shortHelp( char *progName )
{
    ...
    averagingOptions.add_options()
            ...
            ("density0", po::value< Real >(&temp), "value to scale the density in input mass per square Mpc [DEFAULT: use average density].")
